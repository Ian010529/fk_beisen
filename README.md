...
开启“使用 Codex 回答（较慢）”后，扩展仍会优先查询本地题库，只有题库没有达到匹配阈值时才通过已登录的 Codex CLI 分析题干、选项和题图。未开启时只查询本地题库。Codex 在只读临时目录中以无会话持久化方式运行，不需要额外配置 API Key；每题响应时间和可用额度取决于当前 Codex 账户。
