...
开启“使用 Codex 回答（较慢）”后，本地服务会通过已登录的 Codex CLI 分析题干、选项和题图，并将结构化答案送回扩展。Codex 在只读临时目录中以无会话持久化方式运行，不需要额外配置 API Key；每题响应时间和可用额度取决于当前 Codex 账户。

Codex 模式会附带当前可见页面截图，并同时收集题目中的图片 URL、Canvas 和 CSS 背景图。识别结果会显示实际发送给 Codex 的图片数量。
